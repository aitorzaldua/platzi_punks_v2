...
// File @openzeppelin/contracts/token/ERC721/extensions/ERC721Enumerable.sol@v4.4.2
...
contract PlatziPunksDNA {
    ...
    //Pseudorandom DNA number. -> Determinista, no puede usarse en producción.
    //Recibe tokenId y address del usuario
        ...
        returns (uint256)
    {
        ...
        //usamos la funcion keccak256 que crea un hash de una cadena de bytes
        //pasamos primero combinedParams a bytes
        bytes memory encodedParams = abi.encodePacked(combinedParams);
        bytes32 hashedParams = keccak256(encodedParams);

        //retornamos el hash pasado a entero
    ...
    //Recibida la cadena _dna dividirla en pares según cada atributo
    //Recive 2 valores:
    //_dna : la cadena completa
    //_rightDiscard : los pares que vamos eliminando a la derecha
    ...
    {
        return
            uint8 (   //sin esto, el return sería un uint256
    ...
    //funcion para el primer atributo _accessoriesType
    //recibe como atributo el _dna de 26 dígitos y se lo pasa a la funcion _getDNASection
